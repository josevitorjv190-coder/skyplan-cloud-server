...
function trafficFor(room, excludeId, excludeCallsign){
  const seen = new Set();
  return roomClients(room)
    .filter(c=>{
      if(c.id===excludeId) return false;
      if(excludeCallsign && c.callsign && c.callsign.toUpperCase()===excludeCallsign.toUpperCase()) return false;
      if(!c.position || !c.callsign || c.callsign==='undefined') return false;
      // Deduplica por callsign - mantem o mais recente
...
wss.on('connection',(ws,req)=>{
  ...
  ws.on('message',(raw)=>{
    ...
    if(m.type==='xtraffic' && Array.isArray(m.list)){
      // Repassa tráfego para todos os browsers da mesma sala
      for(const cl of roomClients(c.room)){
        if(cl.role==='web' && cl.id!==c.id){
          send(cl.ws,{type:'xtraffic',source:c.callsign,list:m.list});
        }
      }
      return;
    }
